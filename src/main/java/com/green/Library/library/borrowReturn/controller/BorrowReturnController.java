...
public class BorrowReturnController {
    ...
    public Map<String, Object> selectBorrowInfo(@RequestBody Map<String, String> inputData) {
        ...
        if (isNumberic(cardNumOrBookCode) && Integer.parseInt(cardNumOrBookCode) > 0 && !String.valueOf(cardNumOrBookCode).contains("GR")) {
            ...
            int cardNum = Integer.parseInt(cardNumOrBookCode);
    ...
    // 대출
    @ResponseBody
    @PostMapping("/insertBorrowInfoFetch")
    public Map<String, Object> insertBorrowInfoFetch(@RequestBody Map<String, String> inputData, BookBNRVO bookBNRVO, BookReservationVO bookReservationVO) {
        // 데이터 담을 통
        Map<String, Object> returnData = new HashMap<>();

        //input 태그에 입력한 데이터
        String bookCode = inputData.get("inputValue"); // 1
        //받아온 selectedCardNum이 0이면 선택한 카드번호가 없다는 뜻
        int selectedCardNum = Integer.parseInt(inputData.get("selectedCardNum")); // 0

        //유저코드 조회
        int userCode = borrowReturnService.selectUserCode(selectedCardNum);

        //책 번호를 입력했을 경우 카드넘을 통해 이용자 코드를 가져와서 대출
        if (String.valueOf(bookCode).contains("GR") && selectedCardNum != 0) {
    ...
    // 반납
    @ResponseBody
    @PostMapping("/updateReturnFetch")
    public Map<String, Object> updateReturnFetch(@RequestBody Map<String, String> inputData, BookBNRVO bookBNRVO, BookReservationVO bookReservationVO){
        // 데이터 담을 통
        Map<String, Object> returnData = new HashMap<>();
