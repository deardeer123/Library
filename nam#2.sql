CREATE TABLE USERS(
	...
	, USER_TEL VARCHAR(20) UNIQUE NOT NULL
...
ALTER TABLE USERS CHANGE GENDER GENDER VARCHAR(2) NOT NULL;
ALTER TABLE USERS CHANGE EMAIL EMAIL VARCHAR(50) NOT NULL;
DROP TABLE users;

SELECT *FROM users;
UPDATE users
SET email = 'pppp6565@naver.com'
WHERE user_code = 18;
...
SELECT * FROM users;factors_causing_electric_accidentsfactors_causing_electric_accidentsfactors_causing_electric_accidentsfactors_causing_electric_accidents

SELECT * FROM python_range;
...
WHERE BURNRANGE0_5 = (SELECT MAX(BURNRANGE0_5) FROM electric_accidents_burn_range);
...
+BURNRANGE41_50
+BURNRANGE51_60
+BURNRANGE60_over) AS total_range
FROM electric_accidents_burn_range
WHERE occurred_year = 2020;

-- 가장 적은 //
SELECT
occurred_year
FROM electric_accidents_burn_range
WHERE BURNRANGE0_5 = (SELECT MIN(BURNRANGE0_5) FROM electric_accidents_burn_range);
...
SELECT 
TRUNCATE((SELECT AVG(INSULATION_DETERIORATION_SHORT_CIRCUIT) FROM factors_causing_electric_accidents))
FROM factors_causing_electric_accidents;


SELECT 
ROUND(AVG(INSULATION_DETERIORATION_SHORT_CIRCUIT), 0),
ROUND(AVG(TRACKING_SHORT_CIRCUIT),0),
ROUND(AVG(COMPRESSION_DAMAGE_SHORT_CIRCUIT),0),
ROUND(AVG(LAYER_SHORT_CIRCUIT),0),
ROUND(AVG(UNIDENTIFIED_SHORT_CIRCUIT),0),
ROUND(AVG(OVERLOAD_AND_OVERCURRENT),0),
ROUND(AVG(ELECTRIC_LEAKAGE_GROUNDING),0),
ROUND(AVG(BAD_CONTACT),0),
ROUND(AVG(HALF_CUT_LINE),0),
ROUND(AVG(ETC),0)
FROM factors_causing_electric_accidents;
...
SELECT
	...
	+BURNRANGE41_50
	+BURNRANGE51_60
	+BURNRANGE60_OVER) FROM electric_accidents_burn_range WHERE OCCURRED_YEAR = 2016) AS TOTAL
